...
function SignatureRequestsTable({ staffId, staffProgram, year, course, isSuperUser }) {
	...
	const handleStatusUpdate = (id, newStatus) => {
		...
		// Only include staffId when status is 'Approved'
		...
		api
			...
			.then((res) => {
				console.log(res);
				setSignatures((prev) => prev.map((item) => (item.id === id ? { ...item, status: newStatus } : item)));
			})
			.catch((err) => {
							...
							<tbody className="divide-y divide-gray-300">
								{signatures.map((signature) => (
									<tr
										...
										<td className="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">
											{signature.signatureDetail?.year_level} - {signature.signatureDetail?.user?.last_name}
										</td>
										<td className="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">
										...
										<td className="p-5">
											<div className="flex items-center gap-1">
												<button
													onClick={() => handleStatusUpdate(signature.id, "Approved")}
												...
												<button
													onClick={() => handleStatusUpdate(signature.id, "Pending")}
